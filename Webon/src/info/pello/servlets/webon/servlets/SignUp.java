...
import info.pello.servlets.webon.beans.FileList;
import info.pello.servlets.webon.beans.File;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (login.isEmpty() || password.isEmpty() || email.isEmpty()) {
			...
			if (password.equals(password2)) {
				...
				if (!user.checkExists()) {
					int iduser = user.createNew();
					FileList fileList = new FileList();
					// Creamos el directorio inicial del usuario:
					int idhome = fileList.createFile(0, iduser, user.getLogin(), user.getLogin() +" home", null, File.IS_DIR);
						
					System.out.println("OK, nuevo usuario: " + iduser);
